...
import {
  ...
  RefreshCw, Info, ChevronDown, ChevronUp, XCircle,
} from "lucide-react";
          ...
          {site.gallery.length > 0 && (
            <div className="flex gap-2 mb-4">
              {site.gallery.map((g, i) => (
                <img key={i} src={g} alt="" className="flex-1 h-20 object-cover rounded-lg" />
              ))}
            </div>
          )}
            ...
            <div className="px-5 pb-4 border-t border-current/10">
              {b.staffFeedback && (
                <div className="bg-white/60 rounded-lg p-3 my-3">
                  <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
                    <MessageSquare size={11} /> Staff note
                  </p>
              ...
              {b.status === "Rescheduled" && b.suggestedCheckIn && (
                <div className="bg-white/60 rounded-lg p-3 mb-3">
                  <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
                    <Calendar size={11} /> Suggested new dates
                  </p>
                  <p className="text-sm text-foreground font-medium">
                    {fmtDate(b.suggestedCheckIn)} → {fmtDate(b.suggestedCheckOut)}
                  </p>
                </div>
              )}
...
export function CustomerDashboard({
  ...
  // "My bookings" = the demo bookings for Amirah + any newly created ones
  ...
  const tabs: { id: TabId; label: string; badge?: number }[] = [
    ...
    { id: "mybookings", label: "My Bookings", badge: actionableNotifs.length || undefined },
  ];
        ...
        {tab === "browse" && (
          <div>
            ...
            <p className="text-muted-foreground text-sm mb-8">
              Hover over or click a site to learn more, then book your stay.
            </p>
          ...
          <div className="max-w-xl mx-auto">
            ...
            {!selected ? (
                ...
                <div className="bg-muted rounded-lg p-4 mt-5 flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground">
                      {nights} night{nights > 1 ? "s" : ""} · {bookingData.guests} guest{Number(bookingData.guests) > 1 ? "s" : ""}
                    </p>
              ...
              <div className="bg-card border border-border rounded-xl p-6 mt-4">
                ...
                <div className="space-y-3 text-sm mb-6">
                  {[
                    ...
                    ["Type", selected.type],
                    ["Check-in", fmtDate(bookingData.checkIn)],
                    ["Check-out", fmtDate(bookingData.checkOut)],
                    ...
                    <div key={label} className="flex justify-between border-b border-border pb-2">
                      ...
                      <span
                        className="text-foreground"
                        style={label === "Total" ? { fontFamily: "'DM Mono', monospace" } : {}}
                      >
                        {val}
                      </span>
                    </div>
              ...
              <div className="bg-card border border-border rounded-xl p-8 text-center mt-4">
                ...
                <p style={{ fontFamily: "'DM Mono', monospace" }} className="text-accent text-sm mb-6">
                  Ref: {lastBookingId}
                </p>
                <div className="flex gap-3 justify-center">
                  <button
                    onClick={() => { setTab("payment"); setPayStep("select"); }}
                    className="bg-primary text-primary-foreground px-6 py-2 rounded-lg text-sm hover:opacity-90"
                  >
                    Proceed to Payment
                  </button>
            ...
            {payStep === "select" ? (
              <div className="bg-card border border-border rounded-xl p-6 mt-4">
                ...
                <button
                  ...
                  className="w-full bg-primary text-primary-foreground py-3 rounded-lg text-sm hover:opacity-90"
                >
                  Continue
                </button>
              </div>
              ...
              <div className="bg-card border border-border rounded-xl p-6 mt-4">
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" placeholder="MM / YY" className="w-full px-3 py-2 rounded-lg border border-border bg-input-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                      </div>
                      <div>
                        <label className="block text-sm text-foreground mb-1">CVV</label>
                        <input type="text" placeholder="•••" maxLength={3} className="w-full px-3 py-2 rounded-lg border border-border bg-input-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm text-foreground mb-1">Name on Card</label>
                      <input type="text" placeholder="As printed on card" className="w-full px-3 py-2 rounded-lg border border-border bg-input-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                    </div>
                  </div>
                )}
                {payMethod === "fpx" && (
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setPayStep("done")} className="flex-1 bg-accent text-accent-foreground py-2 rounded-lg text-sm hover:opacity-90">Pay Now</button>
                </div>
              </div>
            ) : (
              <div className="bg-card border border-border rounded-xl p-8 text-center mt-4">
                ...
                .map((b) => (
                  <div key={b.id} className="bg-card border border-border rounded-xl p-5">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className={`text-xs px-3 py-1 rounded-full ${statusColor[b.status] ?? "bg-muted text-muted-foreground"}`}>
                        {b.status}
                      </span>
                    </div>
                    <div className="grid grid-cols-3 gap-4 text-sm border-t border-border pt-3">
                      ...
                      <div className="flex items-center gap-1 text-muted-foreground">
                        <CreditCard size={13} />
                        <span className={b.paid ? "text-primary" : "text-accent"}>{b.paid ? "Paid" : "Pending"}</span>
                      </div>
                    </div>
