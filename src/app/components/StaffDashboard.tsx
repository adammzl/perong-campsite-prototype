...
type TabId = "overview" | "availability" | "approvals";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl p-6 max-w-md w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-muted rounded-lg p-3 mb-5 text-sm space-y-1">
          ...
          <p className="text-muted-foreground">{fmtDate(booking.checkIn)} → {fmtDate(booking.checkOut)} · {booking.guests} guests</p>
          ...
          <div className="mb-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="date" value={suggestCheckIn} onChange={(e) => setSuggestCheckIn(e.target.value)}
              ...
              <div>
                ...
                <input type="date" value={suggestCheckOut} onChange={(e) => setSuggestCheckOut(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg border border-border bg-input-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
            </div>
          </div>
        ...
        {(action === "reject" || action === "reschedule") && feedback.trim().length > 0 && feedback.trim().length <= 5 && (
          <p className="text-xs text-destructive mb-3">Please provide a more detailed reason.</p>
        )}

        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 border border-border text-foreground py-2 rounded-lg text-sm hover:bg-muted transition-colors">Cancel</button>
          <button disabled={!canSubmit} onClick={() => onSubmit({ action, feedback, suggestCheckIn, suggestCheckOut })}
            className={`flex-1 py-2 rounded-lg text-sm transition-opacity font-medium disabled:opacity-40 disabled:cursor-not-allowed ${
              action === "approve" ? "bg-primary text-primary-foreground hover:opacity-90"
              : action === "reject" ? "bg-destructive text-white hover:opacity-90"
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        <div>
          <p style={{ fontFamily: "'Playfair Display', serif", fontWeight: 600 }} className="text-foreground">{booking.guest}</p>
          <p style={{ fontFamily: "'DM Mono', monospace" }} className="text-xs text-muted-foreground">{booking.id}</p>
        </div>
        <span className={`text-xs px-3 py-1 rounded-full ${bookingStatusStyle[booking.status] ?? "bg-muted text-muted-foreground"}`}>
          {booking.status}
        </span>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-4">
        <div className="flex items-center gap-1 text-muted-foreground"><TreePine size={13} /><span>{booking.site}</span></div>
        <div className="flex items-center gap-1 text-muted-foreground"><Calendar size={13} /><span>{booking.dates}</span></div>
        <div className="flex items-center gap-1 text-muted-foreground"><Users size={13} /><span>{booking.guests} guests</span></div>
        <div className="flex items-center gap-1 text-muted-foreground"><CreditCard size={13} /><span style={{ fontFamily: "'DM Mono', monospace" }}>RM {booking.total}</span></div>
      </div>
      {booking.staffFeedback && (
        <div className="mb-3">
          <button onClick={() => setShowNote((p) => !p)} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground mb-1">
            <MessageSquare size={11} /> Staff note sent to guest {showNote ? <ChevronUp size={11} /> : <ChevronDown size={11} />}
          </button>
          {showNote && <div className="bg-muted rounded-lg p-3 text-sm"><p className="text-foreground">{booking.staffFeedback}</p></div>}
        </div>
      )}
      {booking.status === "Rescheduled" && booking.suggestedCheckIn && (
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 mb-3 text-sm">
          ...
          <p className="text-foreground">{fmtDate(booking.suggestedCheckIn)} → {fmtDate(booking.suggestedCheckOut)}</p>
        </div>
      )}
      {booking.status === "Pending" && (
        <div className="flex gap-3 pt-3 border-t border-border">
          <button onClick={() => onReview(booking)} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-lg text-sm hover:opacity-90">
...
export function StaffDashboard({
  ...
  // Toggle between booking and payment sub-views within the combined approvals tab
  const [approvalsView, setApprovalsView] = useState<"bookings" | "payments">("bookings");

  const pendingBookings = bookings.filter((b) => b.status === "Pending").length;
  const pendingPayments = payments.filter((p) => p.status === "Pending").length;
  const pendingApprovals = pendingBookings + pendingPayments;
  ...
  const handleBookingAction = (
    bookingId: string,
    { action, feedback, suggestCheckIn, suggestCheckOut }: { action: ActionType; feedback: string; suggestCheckIn: string; suggestCheckOut: string }
  ) => {
  ...
  const tabs: { id: TabId; label: string; badge?: number }[] = [
    ...
    { id: "approvals", label: "Approvals", badge: pendingApprovals || undefined },
  ];
    ...
    <div style={{ fontFamily: "'Lato', sans-serif" }} className="min-h-screen bg-background">
      ...
      <header className="bg-foreground text-primary-foreground px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={onLogout} className="flex items-center gap-1 text-primary-foreground/60 hover:text-primary-foreground text-sm">
        ...
        <div className="flex gap-0 min-w-max">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              {t.label}
              {!!t.badge && (
                <span className="bg-accent text-accent-foreground text-xs rounded-full w-5 h-5 flex items-center justify-center">{t.badge}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {tab === "overview" && (
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
              {[
                { label: "Available Sites", value: String(availableSites), icon: <TreePine size={20} className="text-primary" />, sub: `of ${campsites.length} total` },
                { label: "Pending Bookings", value: String(pendingBookings), icon: <Clock size={20} className="text-accent" />, sub: "awaiting approval" },
                { label: "Pending Payments", value: String(pendingPayments), icon: <AlertTriangle size={20} className="text-accent" />, sub: "to verify" },
              ...
              {bookings.filter((b) => b.status === "Pending").map((b) => (
                <div key={b.id} className="bg-card border border-border rounded-lg px-5 py-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setActionBooking(b)} className="bg-primary/10 text-primary px-3 py-1 rounded text-xs hover:bg-primary hover:text-primary-foreground transition-colors">Review</button>
                  </div>
                </div>
              ))}
              {payments.filter((p) => p.status === "Pending").map((p) => (
                <div key={p.id} className="bg-card border border-border rounded-lg px-5 py-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => approvePayment(p.id)} className="bg-primary/10 text-primary px-3 py-1 rounded text-xs hover:bg-primary hover:text-primary-foreground transition-colors">Approve</button>
                  </div>
                </div>
              ))}
        ...
        {tab === "availability" && (
          <div>
            ...
            <p className="text-muted-foreground text-sm mb-8">Changes here are reflected instantly on the customer side.</p>
            <div className="space-y-3">
              {campsites.map((site) => (
                <div key={site.id} className="bg-card border border-border rounded-xl p-5 flex flex-wrap items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    {editingPrice === site.id ? (
                      <>
                        ...
                        <button onClick={() => savePrice(site.id)} className="text-primary hover:text-primary/80"><Check size={16} /></button>
                        <button onClick={() => setEditingPrice(null)} className="text-muted-foreground hover:text-foreground"><X size={16} /></button>
                      </>
                    ) : (
                      <button onClick={() => { setEditingPrice(site.id); setPriceInput(String(site.price)); }}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => toggleAvailability(site.id)}
        ...
        {tab === "approvals" && (
          <div>
            ...
            <p className="text-muted-foreground text-sm mb-6">Review booking requests and verify payment submissions in one place.</p>

            {/* Sub-toggle */}
            <div className="flex rounded-xl overflow-hidden border border-border mb-8 w-fit">
              <button
                onClick={() => setApprovalsView("bookings")}
                className={`flex items-center gap-2 px-5 py-2.5 text-sm transition-colors ${
                  approvalsView === "bookings" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"
                }`}
              >
                Booking Requests
                {pendingBookings > 0 && (
                  <span className={`text-xs rounded-full w-5 h-5 flex items-center justify-center ${approvalsView === "bookings" ? "bg-white/20 text-white" : "bg-accent text-accent-foreground"}`}>
                    {pendingBookings}
                  </span>
                )}
              </button>
              <button
                onClick={() => setApprovalsView("payments")}
                className={`flex items-center gap-2 px-5 py-2.5 text-sm transition-colors ${
                  approvalsView === "payments" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"
                }`}
              >
                Payment Verifications
                {pendingPayments > 0 && (
                  <span className={`text-xs rounded-full w-5 h-5 flex items-center justify-center ${approvalsView === "payments" ? "bg-white/20 text-white" : "bg-accent text-accent-foreground"}`}>
                    {pendingPayments}
                  </span>
                )}
              </button>
              ...
              <div className="space-y-4">
                {bookings.length === 0 ? (
                  <div className="bg-card border border-border rounded-lg p-8 text-center text-muted-foreground text-sm">
                    <CheckCircle size={32} className="text-primary mx-auto mb-2" />No booking requests yet.
                  </div>
                ) : (
                  bookings.map((b) => <BookingCard key={b.id} booking={b} onReview={setActionBooking} />)
                )}
              </div>
            )}
            ...
            {approvalsView === "payments" && (
              <div className="space-y-4">
                {payments.length === 0 ? (
                  <div className="bg-card border border-border rounded-lg p-8 text-center text-muted-foreground text-sm">
                    <CheckCircle size={32} className="text-primary mx-auto mb-2" />No payments to verify yet.
                  </div>
                ) : (
                  payments.map((p) => (
                    <div key={p.id} className="bg-card border border-border rounded-xl p-5">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm mb-4">
                        <div className="flex items-center gap-1 text-muted-foreground"><CreditCard size={13} /><span>{p.method}</span></div>
                        <div className="flex items-center gap-1 text-muted-foreground"><Calendar size={13} /><span>{p.date}</span></div>
                        <div className="flex items-center gap-1"><span style={{ fontFamily: "'DM Mono', monospace" }} className="text-foreground">RM {p.amount}.00</span></div>
                      </div>
                      {p.status === "Pending" && (
                        <div className="flex gap-3 pt-3 border-t border-border">
                          <button onClick={() => approvePayment(p.id)} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-lg text-sm hover:opacity-90">
                            <CheckCircle size={15} /> Approve Payment
                          </button>
                          <button onClick={() => rejectPayment(p.id)} className="flex items-center gap-2 border border-destructive text-destructive px-4 py-2 rounded-lg text-sm hover:bg-destructive/10">
